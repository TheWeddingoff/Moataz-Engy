...
(function () {
  ...
  (function audio() {
    ...
    function enableFirstInteractionAutoplay() {
      var interactionEvents = ['pointerdown', 'touchstart', 'keydown'];
      function onFirstInteraction() {
        interactionEvents.forEach(function (evt) {
          document.removeEventListener(evt, onFirstInteraction);
        });
        if (!playing && audioReady) startPlayback();
      }
      interactionEvents.forEach(function (evt) {
    ...
    function startPlayback() {
      ...
      function onBlocked() {
        ...
        // Fallback: browsers may block autoplay until first user gesture.
        enableFirstInteractionAutoplay();
      ...
      if (p && typeof p.then === 'function') {
        p.then(onStarted).catch(onBlocked);
      } else {
        onStarted();
      }
    }
    ...
    function startAutoplayRetryLoop() {
    ...
    audio.load();
    // Try immediately on page open, then canplay will retry path if needed.
    attemptAutoplay();
    startAutoplayRetryLoop();
